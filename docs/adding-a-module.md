...
1. Create `src/modules/example/manifest.ts` with a unique ID, metadata, feature definitions, configuration fields, capabilities, and documentation pages.
...
4. Export a `BotModule` from `index.ts`. Declare `featureForComponent` when components belong to feature switches.
