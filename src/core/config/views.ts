...
export async function configurationHome(settings: GuildConfigService, modules: ModuleRegistry, guildId: string, actorId: string) {
  ...
  return { embeds: [new EmbedBuilder().setColor(ui.colors.primary).setTitle("Server Configuration").setDescription(`**Setup:** Complete\n\n${rows.join("\n")}`)], components: [new ActionRowBuilder<StringSelectMenuBuilder>().addComponents(menu), new ActionRowBuilder<ButtonBuilder>().addComponents(new ButtonBuilder().setCustomId(componentId("core", "config", "audit", actorId)).setLabel("Recent Changes").setStyle(ButtonStyle.Secondary), new ButtonBuilder().setCustomId(componentId("core", "config", "reset_prompt", actorId)).setLabel("Reset / Advanced").setStyle(ButtonStyle.Danger))] };
}
...
export async function moduleConfigurationView(settings: GuildConfigService, modules: ModuleRegistry, guildId: string, moduleId: string, actorId: string) {
  ...
  const configuration = module.manifest.config.map((item) => `**${item.label}:** ${item.type === "channel" ? channelLabel(config[item.key]) : item.type === "role-list" ? roleListLabel(config[item.key]) : String(config[item.key] ?? "Not configured")}`).join("\n") || "No module settings.";
...
export function fieldEditor(modules: ModuleRegistry, moduleId: string, key: string, actorId: string) {
  ...
  if (definition.type === "channel" || definition.type === "channel-list") return { embeds: [new EmbedBuilder().setColor(ui.colors.primary).setTitle(definition.label).setDescription(definition.description)], components: [new ActionRowBuilder<ChannelSelectMenuBuilder>().addComponents(new ChannelSelectMenuBuilder().setCustomId(id).setPlaceholder(definition.label).setChannelTypes(ChannelType.GuildText).setMinValues(definition.required ? 1 : 0).setMaxValues(definition.type === "channel" ? 1 : 10))] };
