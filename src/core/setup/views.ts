...
export async function hasSetupConfiguration(settings: GuildConfigService, registry: ModuleRegistry, guildId: string, moduleId: string): Promise<boolean> {
  for (const definition of registry.require(moduleId).manifest.config) {
    if (await settings.isConfigAvailable(guildId, moduleId, definition.key)) return true;
  }
  return false;
}
