...
export function channelLabel(value: unknown): string { return typeof value === "string" && value ? `<#${value}>` : "No channel selected."; }
export function roleListLabel(value: unknown): string { return Array.isArray(value) && value.length ? value.map((id) => `<@&${String(id)}>`).join(", ") : "No role selected."; }
