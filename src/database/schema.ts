...
export const moderationConfigs = pgTable("moderation_configs", {
  ...
  dmUsers: boolean("dm_users").notNull().default(true),
  rulesUrl: text("rules_url"),
  purgeConfirmationThreshold: integer("purge_confirmation_threshold").notNull().default(25),
  staffRoleIds: jsonb("staff_role_ids").$type<string[]>().notNull().default([]),
  reasonPresets: jsonb("reason_presets").$type<string[]>().notNull().default(["Spam", "Harassment", "Advertising", "Inappropriate content", "Rule evasion", "Staff discretion"]),
  notesEnabled: boolean("notes_enabled").notNull().default(true),
  temporaryBansEnabled: boolean("temporary_bans_enabled").notNull().default(true),
  caseButtonsEnabled: boolean("case_buttons_enabled").notNull().default(true),
  updatedAt: timestamp("updated_at", { withTimezone: true }).notNull().defaultNow(),
});
...
export type ModerationConfig = typeof moderationConfigs.$inferSelect;
