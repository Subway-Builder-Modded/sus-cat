import {
  ActionRowBuilder, ButtonBuilder, ButtonStyle, ChannelType, EmbedBuilder, MessageFlags, ModalBuilder, TextInputBuilder, TextInputStyle,
...
export async function handleModerationComponent(client: BotClient, interaction: RoutedComponentInteraction, action: string, parts: readonly string[]): Promise<void> {
  ...
  if (interaction.isButton()) await routeButton(client, interaction, action, mutableParts);
  else if (interaction.isStringSelectMenu()) await routeSelect(client, interaction, action, mutableParts);
  else await routeModal(client, interaction, action, mutableParts);
}
...
async function routeButton(client: BotClient, interaction: ButtonInteraction<"cached">, action: string, parts: string[]): Promise<void> {
  ...
  if (action === "confirm" || action === "cancel") {
    ...
    const featureId = payload.type === "purge" ? "purge" : payload.type === "softban" ? "softbans" : "bans";
    if (!await client.platform.settings.isFeatureEnabled(interaction.guildId, "moderation", featureId)) throw new Error("That moderation feature was disabled before this action was confirmed.");
...
async function routeModal(client: BotClient, interaction: ModalSubmitInteraction<"cached">, action: string, parts: string[]): Promise<void> {
  ...
  if (action === "modal_evidence_message") {
    ...
    await respond(interaction, { embeds: [successEmbed("Evidence added", `The message was attached to case #${item.caseNumber}.`)] });
    return;
  }
  ...
  if (action === "modal_case_edit") {
    ...
    await respond(interaction, { embeds: [buildCaseEmbed(updated)] });
    ...
    await respond(interaction, { embeds: [successEmbed("Note added", `A private staff note was linked to case #${item.caseNumber}.`)] });
    ...
    await respond(interaction, { embeds: [successEmbed("Evidence added", `Evidence was attached to case #${item.caseNumber}.`)] });
  }
}
