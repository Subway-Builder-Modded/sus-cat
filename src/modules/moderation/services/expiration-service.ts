  ...
  private async process(client: BotClient): Promise<void> {
    ...
    try {
      ...
      for (const action of actions) {
        try {
          if (!await client.platform.settings.isGuildActive(action.guildId) || await client.platform.settings.setupStatus(action.guildId) !== "configured" || !await client.platform.settings.isFeatureEnabled(action.guildId, "moderation", "temporary-bans")) {
            await this.cases.releaseScheduled(action.id);
            continue;
          }
